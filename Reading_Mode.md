...
Click a highlight to focus it, then press {% kbd delete %} or {% kbd backspace %} to remove it. Control-click a highlight and choose **Share...** to open the macOS Share sheet with the document title and highlighted text.
...
With a signature, generated markup uses `{==highlighted text==}{>>signature<<}`. Without one, Marked creates only the `{==highlighted text==}` marker.
