# Marked Help — review handoff (it)

**Locale:** Italian (`it`)  
**Generated:** 2026-07-02  
**Status:** Machine-translated drafts — human review required

This package is ready for post-editing. Work in the `it/` folder on a
`translate/it` (or `locale/it`) branch; open one pull request per locale.
...
| Item | Location |
|------|----------|
| Topic pages (MT drafts) | `it/*.md` (2 files) |
| Help index metadata | `it/config.yaml` |
| Style guide | `it/SYNTAX.md` |
| Term glossary | `it/GLOSSARY.md` |
| Menu path reference | `it/APPMENU.md` |
| App UI strings (read-only reference) | `ui-strings/it-ui-review.marked-l10n` |
| Screenshot folder (optional) | `it/images/` (mirror English paths) |
...
3. [`GLOSSARY.md`](GLOSSARY.md) — agreed EN → Italian terms
4. [`APPMENU.md`](APPMENU.md) — `{% appmenu %}` paths vs the localized app
5. [`ui-strings/it-ui-review.marked-l10n`](../ui-strings/it-ui-review.marked-l10n) — Settings/menus wording to match
...
Edit in place under `it/config.yaml`:
...
- [ ] Branch contains **only** `it/` (and `ui-strings/` if UI wording changed)
...
```bash
python3 HelpDocs/content/scripts/bootstrap_locales.py it
python3 HelpDocs/content/scripts/machine_translate_help.py --locale it
python3 HelpDocs/content/scripts/generate_help_review_handoff.py it
```
