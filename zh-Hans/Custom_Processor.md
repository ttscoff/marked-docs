...
搜索和替换
：对文件内容执行搜索和替换。
：启用 **Regex** 复选框可将搜索字段视为 ICU 正则表达式。启用 Regex 后，`^` 和 `$` 默认匹配每行的开头和结尾。正斜杠 (`/`) 无需转义。如果搜索值仍被 `/.../` 包裹，外层斜杠会被移除一次。
：在模式开头用 `(?…)` 添加内联标志，例如 `(?smi)pattern`。常用标志：`i` 不区分大小写，`m` 多行（`^`/`$` 按行），`s` dotall（`.` 匹配换行）。可组合任意子集（`(?i)`、`(?sm)`、`(?smi)`、…）。在组内用 `-` 前缀可关闭标志（例如 `(?-i)`）。
：关闭 Regex 时，用正斜杠包围搜索字符串（例如 `/Project \w+/`）仍会选择 regex 模式（旧版形式）。在替换字符串中使用 `$1`、`$2` 等表示匹配组。
：替换字段支持一些转义序列（反斜杠后接）：`\n` 插入换行符，`\t` 插入制表符，`\\` 插入字面反斜杠，`\$` 插入字面美元符号（而不是匹配组）
：任何其他 `\X` 序列都被视为 `X`（反斜杠被删除），因此 `\e` 变为 `e`。末尾无后续字符的 \ 会保留为字面反斜杠。
：在替换字符串中使用 `[%key]` 插入来自文档元数据、环境变量或上下文的值（例如 `[%title]`、`[%MARKED_PATH]`）。同一规则中较早操作或先前规则设置的键可用。不匹配的键将替换为空字符串。
：示例：要删除 iA Writer 风格的 `//` 注释行，请启用 **Regex** 和 **All**，将搜索设为 `^//.*$`，并将替换留空。
