...
阅读模式可以保留您在长文档中的位置，聚焦当前块，并让您保存持久的高亮。
...
- {% kbd j %} 或 {% kbd down %}：移至下一个阅读单元。
- {% kbd k %} 或 {% kbd up %}：移至上一个阅读单元。
- {% kbd h %}：突出显示所选内容，或在未选择文本时切换当前阅读单元的突出显示。
...
单击预览顶部的"专注模式"工具可以使除当前阅读单位之外的每个块变暗。导航时，专注模式会跟随当前单位。再次单击该工具可恢复其他块，或离开阅读模式以自动清除专注模式。

## 创建和编辑高亮 [creating-and-editing-highlights]
...
## TextBundle 高亮 [textbundle-highlights]
