...
搜尋和替換
：對文件內容執行搜尋和取代。
：啟用 **Regex** 核取方塊可將搜尋欄位視為 ICU 正規表示式。啟用 Regex 後，`^` 和 `$` 預設比對每行的開頭和結尾。正斜線 (`/`) 無需逸出。如果搜尋值仍被 `/.../` 包裹，外層斜線會被移除一次。
：在模式開頭用 `(?…)` 新增內嵌旗標，例如 `(?smi)pattern`。常用旗標：`i` 不區分大小寫，`m` 多行（`^`/`$` 按行），`s` dotall（`.` 比對換行）。可組合任意子集（`(?i)`、`(?sm)`、`(?smi)`、…）。在群組內用 `-` 前綴可關閉旗標（例如 `(?-i)`）。
：關閉 Regex 時，用正斜線包圍搜尋字串（例如 `/Project \w+/`）仍會選擇 regex 模式（舊版形式）。在替換字串中使用 `$1`、`$2` 等表示匹配組。
：替換欄位支援一些逸出序列（反斜線後接）：`\n` 插入換行符，`\t` 插入定位字元，`\\` 插入字面反斜線，`\$` 插入字面美元符號（而不是匹配組）
：任何其他 `\X` 序列都被視為 `X`（反斜線被刪除），因此 `\e` 變為 `e`。末尾無後續字元的 \ 會保留為字面反斜線。
：在替換字串中使用 `[%key]` 插入來自文件元資料、環境變數或上下文的值（例如 `[%title]`、`[%MARKED_PATH]`）。同一規則中較早操作或先前規則設定的鍵可用。不匹配的鍵將替換為空字串。
：範例：要刪除 iA Writer 風格的 `//` 註解行，請啟用 **Regex** 和 **All**，將搜尋設為 `^//.*$`，並將替換留空。
