...
## 使用共享選單 [using-the-share-menu]

### 從 Finder [from-finder]
...
## 啟用共享延伸功能 [enable-the-share-extension]

Marked 必須安裝在 `/Applications`（或您常用的應用程式資料夾）中，且至少啟動一次，macOS 才會列出其共享延伸功能。

### 在系統設定中啟用 Marked [turn-on-marked-in-system-settings]
...
### 將 Marked 加入 App 的共享選單 [add-marked-to-an-apps-share-menu]
...
## 若共享選單中沒有 Marked [if-marked-does-not-appear-in-share]
